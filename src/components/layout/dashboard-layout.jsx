...
import { Sun, Moon, Menu, X, Briefcase, User, LogOut, LayoutDashboard, Search, Bookmark, FileText, Users, Building2, BarChart3, Tags, ChevronLeft, ChevronRight, } from "lucide-react";
...
export function DashboardLayout({ children }) {
    ...
    return (<div className="min-h-screen bg-background transition-theme">
      {/* Sidebar - Desktop */}
      <aside className={cn("fixed left-0 top-0 z-40 h-screen border-r border-border bg-sidebar transition-all duration-300 hidden lg:block", sidebarOpen ? "w-64" : "w-20")}>
        <div className="flex h-full flex-col">
          ...
          <div className="flex h-16 items-center justify-between px-4 border-b border-sidebar-border">
            ...
            <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(!sidebarOpen)} className="h-8 w-8 text-sidebar-foreground">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            ...
            return (<Link key={item.href} to={item.href} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", isActive
          ...
          <div className="border-t border-sidebar-border p-4">
            ...
            <div className={cn("flex gap-2 mt-4", !sidebarOpen && "flex-col items-center")}>
              <Button variant="ghost" size="icon" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="h-9 w-9 text-sidebar-foreground">
                {mounted && theme === "dark" ? <Sun className="h-4 w-4"/> : <Moon className="h-4 w-4"/>}
              </Button>
              <Button variant="ghost" size="icon" onClick={logout} className="h-9 w-9 text-sidebar-foreground hover:text-destructive">
          ...
          <aside className="absolute left-0 top-0 h-full w-72 border-r border-border bg-sidebar">
            ...
            <nav className="px-3 py-4 space-y-1">
              {items.map((item) => {
                const Icon = item.icon;
                const isActive = pathname === item.href;
                return (<Link key={item.href} to={item.href} onClick={() => setMobileOpen(false)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", isActive
                        ? "bg-sidebar-accent text-sidebar-accent-foreground"
                        : "text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-sidebar-accent/50")}>
